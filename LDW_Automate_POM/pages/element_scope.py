...
class Page_scope(Base_pages):
    ...
    #右上角角色
    now_user=(By.XPATH,u'//*[@id="mainApp"]/div/div[1]/div[2]/div/ul/li[2]/div[1]/span')
    #新建角色
    ...
    #判断是否创建成功
    rule_new = (By.XPATH,'//*[contains(text(),"权限测试角色")]')  # -----判断
    #搜索框输入新建的角色
    ...
    rule_list=(By.XPATH,u'//*[contains(text(),"sunjianing")]/../../td[5]/div/div/div')
    #点击搜索按钮
    search_rule=(By.XPATH,u'//*[@id="baseApp"]/div/div/div/div[1]/div[1]/div[2]/div[1]/div/div/div/div')
    #权限移除按钮
    ...
    #勾选所有功能权限
    all_scope=(By.XPATH,'//*[@id="baseApp"]/div/div/div/div[2]/div/div[2]/div[1]/div/div/div/div/div[2]/div[3]/div/div[1]/div/label/span[1]/span')
    has_choose=(By.CSS_SELECTOR,".all-checked-checkbox.is-checked")
    #保存权限
    ...
    def change_scope_rulename(self):
        self.sleep_wait(1)
        self.Aclick(self.open_menu)  # 点开折叠菜单
        self.Aclick(self.os_element)  # 点击系统按钮
        self.Aclick(self.os_scope)    #点击角色权限管理
        self.sleep_wait(1)
        ...
        is_exists="该角色名称已存在！"
        try:
            if self.get_text(self.rulername_isexist) in is_exists:  #判断角色是否重复
        ...
        self.sleep_wait(1)
        self.input(self.search_rulename,self.rule_name)  #搜索输入新创建的角色名称
        self.Aclick(self.new_rulename)    #选择新创建的角色
        self.sleep_wait(2)
        self.Aclick(self.relevance_rule)  #选择关联员工
        #self.changes_handles(-1)
        ...
        try:
            if self.get_text(self.no_date) in is_rule:     #当选择员工数据为空时
                print("角色已绑定过")
                self.Aclick(self.cancel_rulename)         #点击标题框
                self.changes_handles(-1)
                self.sleep_wait(1)
                self.Aclick(self.chose_rulename_cancel)   #点击取消关联员工
        except:
            self.Aclick(self.loadname_text)    #点击下拉框中的用户名
            self.Aclick(self.cancel_rulename)  #点击标题框
            self.changes_handles(-1)
            self.sleep_wait(1)
            self.Aclick(self.makesure_loadname)              #确认选择角色名称
        ...
        self.sleep_wait(1)
        self.Aclick(self.new_rulename)  # 选择新创建的角色
        self.Aclick(self.get_rule_scope)  #切换角色权限
        self.sleep_wait(2)
        #if self.Find_element(self.all_scope).is_selected():
        if self.Find_element(self.has_choose):
            print("角色全选成功")
        else:
            self.js_click(self.all_scope)  # 点击全选
            self.Aclick(self.save_all_scope)  # 保存权限
